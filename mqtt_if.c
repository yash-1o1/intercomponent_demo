...
void MQTTClientCallback(int32_t event, void *metaData, uint32_t metaDateLen, void *data, uint32_t dataLen)
{
    ...
    switch((MQTTClient_EventCB)event)
    {
        ...
        case MQTTClient_RECV_CB_EVENT:
        {
            ...
            memcpy(queueElement.topic, receivedMetaData->topic, receivedMetaData->topLen);
            queueElement.topic[receivedMetaData->topLen] = 0;
            memcpy(queueElement.payload, data, dataLen);
            queueElement.payload[dataLen] = 0;
            LOG_DEBUG("Received topic: %s\r\n", queueElement.topic);
